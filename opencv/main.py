...
def enhance_linear_contrast(image, alpha, beta):
	...
	subplots = [121, 122]
	titles = ["Original Image" ,"Linear Constrast Image"]
	display_images(images, subplots, titles)
...
def enhance_piecewise_linear(image, x1, x2, k1, k2, y1):
	img_float32 = np.float32(image)
	dst = img_float32
	condlist = [(dst < x1), dst >= x2]
	funclist = [lambda dst:k1*dst + y1 - k1*x1, lambda dst:k2*dst + y1 + k2*x2]
	dst = np.piecewise(dst, condlist, funclist)
